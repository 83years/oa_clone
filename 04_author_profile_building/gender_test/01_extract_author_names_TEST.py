#!/usr/bin/env python3
"""
Step 1: Extract Author Names and Country Codes from PostgreSQL - TEST VERSION

This is a test version limited to 1,000 authors for initial validation.

Identical to 01_extract_author_names.py but with LIMIT clauses added to queries.
"""
...
class Config:
    ...
    # **TEST MODE: Limit to 1,000 authors**
    TEST_LIMIT = 1000
    ...
    # Output paths (test version)
    OUTPUT_DIR = os.path.join(os.path.dirname(__file__), 'output')
    OUTPUT_FILE = os.path.join(OUTPUT_DIR, '01_extracted_names_TEST.json')

    # Logging
    LOG_DIR = os.path.join(os.path.dirname(__file__), 'logs')
    LOG_FILE = os.path.join(LOG_DIR, f'01_extract_names_TEST_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
...
def setup_logging():
    ...
    logging.info("*** TEST MODE: LIMITED TO 1,000 AUTHORS ***")
...
def extract_authors(conn: psycopg2.extensions.connection) -> List[Dict]:
    ...
    **TEST VERSION: LIMITED TO 1,000 AUTHORS**
    ...
    logging.info(f"Loaded {len(authors):,} authors with display names (TEST LIMIT: {Config.TEST_LIMIT})")

    return authors


def extract_name_variants(conn: psycopg2.extensions.connection, author_ids: List[str]) -> List[Dict]:
    """
    Extract alternative names from author_name_variants table
    **TEST VERSION: ONLY FOR TEST AUTHORS**
    ...
    if not author_ids:
        logging.warning("No author IDs provided, skipping variants")
        return []
    ...
    # Only get variants for our test authors
    query = """
        SELECT
            author_id,
            alternative_name
    ...
    logging.info(f"Loaded {len(variants):,} name variants for test authors")

    return variants


def extract_country_codes(conn: psycopg2.extensions.connection, author_ids: List[str]) -> Dict[str, str]:
    """
    ...
    **TEST VERSION: ONLY FOR TEST AUTHORS**
    ...
    if not author_ids:
        logging.warning("No author IDs provided, skipping country codes")
        return {}
    ...
    # Only get countries for our test authors
    query = """
        SELECT DISTINCT
            ai.author_id,
            i.country_code
    ...
    logging.info(f"Loaded {len(results):,} author-country associations for test authors")
...
def extract_author_names() -> List[Dict]:
    """
    Main extraction function - TEST VERSION

    Returns:
        List of dicts with author_id, forename, country_code
    """
    logging.info("=== STEP 1: EXTRACT AUTHOR NAMES (TEST) ===")
    ...
    try:
        ...
        variants = extract_name_variants(conn, author_ids)
        country_codes = extract_country_codes(conn, author_ids)
        ...
        for i, record in enumerate(final_data[:10], 1):
            logging.info(f"{i}. {record['author_id']}: {record['forename']} ({record['country_code'] or 'no country'})")
...
def main():
    ...
    logging.info("Starting name extraction (TEST VERSION)...")
    logging.info(f"Configuration:")
    logging.info(f"  Database: {Config.DB_CONFIG['database']}@{Config.DB_CONFIG['host']}")
    logging.info(f"  Test limit: {Config.TEST_LIMIT} authors")
    ...
    try:
        ...
        logging.info("=== STEP 1 COMPLETE (TEST) ===\n")
