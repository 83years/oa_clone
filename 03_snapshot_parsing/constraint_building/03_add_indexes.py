...
class IndexBuilder:
    ...
    def add_all_indexes(self, scope='all'):
        """
        Add indexes to database tables

        Args:
            scope: Which indexes to create ('all', 'authorship', 'keywords')
        """
        self.log("\n" + "="*70)
        self.log(f"ADDING INDEXES - SCOPE: {scope.upper()}")
        self.log("="*70 + "\n")

        # Define indexes by scope
        authorship_fk_indexes = [
            # Authorship tables (CRITICAL for career trajectory calculations)
            ('authorship', 'author_id'),
            ('authorship', 'work_id'),

            # Authorship institutions (separate table, not authorship.institution_id!)
            ('authorship_institutions', 'author_id'),
            ('authorship_institutions', 'work_id'),
            ('authorship_institutions', 'institution_id'),

            # Authors works by year
        ...
        # Create indexes based on scope
        self.log("PHASE 1: Foreign Key Column Indexes")

        if scope in ['authorship', 'all']:
            self.log("\n  Creating authorship-related indexes...")
            for table_name, column_name in authorship_fk_indexes:
                self.create_index(table_name, column_name)

        if scope in ['keywords', 'all']:
            self.log("\n  Creating keyword-related indexes...")
            for table_name, column_name in keyword_fk_indexes:
                self.create_index(table_name, column_name)

        if scope == 'all':
            self.log("\n  Creating other foreign key indexes...")
            for table_name, column_name in other_fk_indexes:
                self.create_index(table_name, column_name)

        # Common query indexes
        self.log("\nPHASE 2: Common Query Column Indexes")

        authorship_query_indexes = [
            # Works table (only fields needed for career trajectory)
            ...
            ('authors', 'current_affiliation_id'),
        ...
        keyword_query_indexes = [
            # For text search (when needed later)
            # Note: GIN indexes for full-text search will be added separately
        ]

        other_query_indexes = [
            # Works table (additional fields)
        ...
        if scope in ['authorship', 'all']:
            self.log("\n  Creating authorship query indexes...")
            for table_name, column_name in authorship_query_indexes:
                self.create_index(table_name, column_name)

        if scope in ['keywords', 'all']:
            self.log("\n  Creating keyword query indexes...")
            for table_name, column_name in keyword_query_indexes:
                self.create_index(table_name, column_name)

        if scope == 'all':
            self.log("\n  Creating other query indexes...")
            for table_name, column_name in other_query_indexes:
                self.create_index(table_name, column_name)

        # Composite indexes for common queries
        self.log("\nPHASE 3: Composite Indexes for Common Queries")

        authorship_composite_indexes = [
            # Critical for career trajectory calculations
            ...
            # Critical for authors_works_by_year queries
            ('authors_works_by_year', 'author_id, year', 'idx_awby_author_year'),
            ('authors_works_by_year', 'year', 'idx_awby_year'),

            # Productivity metrics
            ('authors', 'works_count, cited_by_count', 'idx_authors_productivity'),
        ]

        keyword_composite_indexes = [
            # Can add composite indexes for keyword searches here later if needed
        ]

        other_composite_indexes = [
            ('works', 'publication_year, cited_by_count', 'idx_works_year_citations'),
        ]

        if scope in ['authorship', 'all']:
            self.log("\n  Creating authorship composite indexes...")
            for table_name, columns, index_name in authorship_composite_indexes:
                self.create_index(table_name, columns, index_name)

        if scope in ['keywords', 'all']:
            self.log("\n  Creating keyword composite indexes...")
            for table_name, columns, index_name in keyword_composite_indexes:
                self.create_index(table_name, columns, index_name)

        if scope == 'all':
            self.log("\n  Creating other composite indexes...")
            for table_name, columns, index_name in other_composite_indexes:
                self.create_index(table_name, columns, index_name)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Add Indexes to Database',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Scope options:
  all        - Create all indexes (default)
  authorship - Only create indexes for authorship, authors, works tables (for career trajectory)
  keywords   - Only create indexes for keyword/search tables

Examples:
  python add_indexes.py --scope authorship    # Only authorship-related indexes
  python add_indexes.py --scope keywords      # Only keyword-related indexes
  python add_indexes.py --scope all           # All indexes
  python add_indexes.py --test --scope authorship  # Authorship indexes on test DB
        """
    )
    ...
    try:
        success = builder.add_all_indexes(scope=args.scope)
